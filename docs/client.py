...
class GUI:
    ...
    def login(self,so):
        message = (self.name_widget.get()+'-'+self.password_widget.get()).encode()
        so.send(message)
    ...
    def display_name_section(self):
        ...
        self.join_button = Button(frame,text="加入群聊", width=10, command=self.on_join).pack(side='left',padx=5)
    ...
    def on_join(self):
        ...
        self.login(self.client_socket.client_socket)
